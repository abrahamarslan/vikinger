...
trait FtopicStoreTrait
{
    use ImageUploadTrait;
    public function save(FtopicStoreRequest $request, $create = false, $record = null) {
        try {
            $image = null;
            $seoImage = null;
            if($create) {
                $item                           =   Ftopic::create($request->except(['image','seo_image']));
            } else {
            ...
            if($request->hasFile('image')) {
                $image                              =   $this->uploadOne(
                    $request,
                    public_path() .  \Config::get('global.ftopic.upload_folder_path_original'),
                    public_path() . \Config::get('global.ftopic.upload_folder_path_resized'),
            ...
            if($request->hasFile('seo_image')) {
                $seoImage                            =   $this->uploadOne(
                    $request,
                    public_path() .  \Config::get('global.ftopic.upload_folder_seo_path_original'),
                    public_path() . \Config::get('global.ftopic.upload_folder_seo_path_resized'),
                    \Config::get('global.image_resize.category_imageX'),
                    \Config::get('global.image_resize.category_imageY'),
                    'seo_image'
                );
